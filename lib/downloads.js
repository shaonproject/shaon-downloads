...
function Instagram(url){
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (hasil.every(x => x === undefined)) return resolve({ developer: '@shaon', status: 404, mess: 'No result found', result: { link: 'https://i.ibb.co/G7CrCwN/404.png', slide: ['https://i.ibb.co/G7CrCwN/404.png'], slide_length: 404 } })
      const hsil = {
        link: hasil[0],
        slide: hasil,
...
function MusiCally(url){
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (typeof vdlk === 'undefined') return resolve({ developer: '@shaonproject', mess: 'Infalid link, no result found' })
  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        if (typeof link === "undefined") return resolve({ creator: "@shaonproject", mess: `Link Not Found` });
        resolve({url: link})
      } else {
        console.log('No result found')
      }
  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        if (typeof link === 'undefined') return resolve({ developer: '@shaonproject', mess: 'no result found' })
...
function TikTok(url){
  return new Promise(async(resolve, reject) => {
    try {
      ...
      const { data, status } = await axios("https://www.tikwm.com/api", {
        method: "POST",
        data: new URLSearchParams(Object.entries(config)),
      });
      if (data.mess) return resolve({ developer: '@shaonproject', mess: data.mess });
      let ar = []
      let aud = []
      let wm = []
  ...
  return new Promise(async(resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        resolve({ developer: '@shaonproject', mess: 'no result found' })
      }
    } catch (error) {
      console.error(error)
    }
